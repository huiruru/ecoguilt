...
For the recommended model, look up its entry in the model cache by `id`, then calculate using pricing ratio scaling (more accurate than token-based estimates due to context caching):
- cur_avg_price = (current input_price + current output_price) / 2
- rec_avg_price = (recommended input_price + recommended output_price) / 2
- cost = state.cost_usd × (rec_avg_price / cur_avg_price)
- kWh = state.total_kwh × (rec_avg_price / cur_avg_price)
